# 本文件定义 Agent Server 的结构化接口审计日志写入器。
...
from travel_agent_agent.core.correlation import CorrelationContext


def create_audit_logger() -> logging.Logger:
    """创建幂等的标准输出审计 logger，供 Docker 日志采集安全查询。"""
    logger = logging.getLogger("travel_agent_agent.audit")
...
def log_interface_event(
    ...
    """输出单行 JSON 审计日志，禁止接收请求原文、提示词或认证信息。"""
    logger.info(
        "interface_audit %s",
        json.dumps(
            {
