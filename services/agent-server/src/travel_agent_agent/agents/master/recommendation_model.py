# 文件职责：创建通过 Tool Gateway 调用的 qwen3.6-flash 推荐裸模型。
# 定义 create_recommendation_chat_model 和 _read_gateway_token，禁止工具与思考模式。
...
def create_recommendation_chat_model(settings: Settings) -> ChatOpenAI:
